// automatically generated by the FlatBuffers compiler, do not modify

package flatmath

import (
	flatbuffers "github.com/google/flatbuffers/go"
)

/// 4D hypercomplex of ubytes
type Quaternion_u8 struct {
	_tab flatbuffers.Struct
}

func (rcv *Quaternion_u8) Init(buf []byte, i flatbuffers.UOffsetT) {
	rcv._tab.Bytes = buf
	rcv._tab.Pos = i
}

func (rcv *Quaternion_u8) Table() flatbuffers.Table {
	return rcv._tab.Table
}

func (rcv *Quaternion_u8) X() byte {
	return rcv._tab.GetByte(rcv._tab.Pos + flatbuffers.UOffsetT(0))
}
func (rcv *Quaternion_u8) MutateX(n byte) bool {
	return rcv._tab.MutateByte(rcv._tab.Pos+flatbuffers.UOffsetT(0), n)
}

func (rcv *Quaternion_u8) Y() byte {
	return rcv._tab.GetByte(rcv._tab.Pos + flatbuffers.UOffsetT(1))
}
func (rcv *Quaternion_u8) MutateY(n byte) bool {
	return rcv._tab.MutateByte(rcv._tab.Pos+flatbuffers.UOffsetT(1), n)
}

func (rcv *Quaternion_u8) Z() byte {
	return rcv._tab.GetByte(rcv._tab.Pos + flatbuffers.UOffsetT(2))
}
func (rcv *Quaternion_u8) MutateZ(n byte) bool {
	return rcv._tab.MutateByte(rcv._tab.Pos+flatbuffers.UOffsetT(2), n)
}

func (rcv *Quaternion_u8) W() byte {
	return rcv._tab.GetByte(rcv._tab.Pos + flatbuffers.UOffsetT(3))
}
func (rcv *Quaternion_u8) MutateW(n byte) bool {
	return rcv._tab.MutateByte(rcv._tab.Pos+flatbuffers.UOffsetT(3), n)
}

func CreateQuaternion_u8(builder *flatbuffers.Builder, x byte, y byte, z byte, w byte) flatbuffers.UOffsetT {
	builder.Prep(1, 4)
	builder.PrependByte(w)
	builder.PrependByte(z)
	builder.PrependByte(y)
	builder.PrependByte(x)
	return builder.Offset()
}
